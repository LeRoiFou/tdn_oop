"""
Cours : La Programmation Orientée Objet en Python - 10/11 - L'héritage en Python [5/5]
Lien : https://www.youtube.com/watch?v=MHPOsJ5wd7E&list=PLlxQJeQRaKDSB3VOLR8s0vEMHfvUzgqfW&index=10

TDN POO leçon 10 : héritages 

Moins de méthodes dans la classe dérivée FootballTeam : 

La méthode from_string() n'est plus redéfinie dans la classe dérivée FootballTeam car pour cette classe, il y avait 4 
valeurs (nom, victoires, défaites, match nuls) au lieu et place de 3 valeurs (nom, victoires et défaites) : pour cela, 
dans la classe parente SportTeam, on a eu recours à l'instruction '*' dans la méthode from_string() ainsi qu'une 
intervention dans la méthode statique pluriel() du module utilities pour convertir les str en int. 

De même, le constructeur de la classe parente redéfinie dans la classe dérivée FootballTeam est supprimée. En 
contrepartie, dans la classe parente, on rajoute en paramètre du constructeur l'attribut draws avec pour valeur 0. 
...
class StatsFormatError(Exception):  # Création de sa propre exception appliquée dans la méthode de classe 'from_string'
    pass
...
class SportTeam:
    ...
    def __init__(self, name, wins, losses, draws=0):  # constructeur
        """ Les variables ci-après sont des variables d'instance car chaque équipe (chaque objet) a son propre nom,
        ses propres victoires et ses propres défaites : ces variables sont uniques à chaque équipe. Rajout de l'attribut
        draws qui est uniquement utilisé dans la classe dérivée FootballTeam, et dans ce cas on paramètre cet attribut
        dans le constructeur avec pour valeur 0 afin que celui-ci s'applique uniquement pour la classe dérivée
        FootballTeam """
        self.name = name
        self.wins = wins
        self.losses = losses
    ...
    def from_string(cls, stats_as_string):  # cette méthode converti des données déclarées sous la forme de str
        try:
            return cls(*stats_as_string.split('-'))  # conversion de la str en liste avec pour séparateur le '-'
        except ValueError:
...
class FootballTeam(SportTeam):  # la classe 'FootballTeam' hérite de la classe 'SportTeam'
    ...
    """ Seule la méthode stats() issue de la super-classe 'SportTeam' est redéfinie car il doit-être mentionné
    l'intitulé de [FOOTBALL] STATS, ainsi que les matchs nuls obtenus pour chaque équipe de football """
