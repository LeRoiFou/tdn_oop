"""
Cours : La Programmation Orientée Objet en Python - 8/11 - L'héritage en Python [3/5]
Lien : https://www.youtube.com/watch?v=KGBWir2Tgaw&list=PLlxQJeQRaKDSB3VOLR8s0vEMHfvUzgqfW&index=8

TDN POO leçon 8 : héritages
Redéfinition de méthodes dans une classe dérivée.

L'objectif ici est de redéfinir le constructeur de la super-classe 'SportTeam' dans la classe dérivée 'FootballTeam'
car dans un jeu de basketball il ne peut pas y avoir de match nul -> il y a une victoire pour équipe et une défaite pour
l'autre. Donc pour une équipe de Football il faut afficher en plus des victoires et des défaites, les matchs nuls.

Le constructeur redéfinie dans la classe dérivée 'FootballTeam' sera donc déclarée de la manière suivante :
super().__init__(name, wins, losses)
Mais dans le cas où il y a de multiples héritages, à savoir qu'une classe hérite d'attributs et de méthodes de plusieurs
classes parentes, dans ce cas, l'instruction pour redéfinir un constructeur d'une classe parente est la suivante :
NomClasseParent.__init(self, attribut1, attribut2...)
...
class SportTeam:
    ...
    def from_file(cls, stats_as_file):  # cette méthode converti des données déclarées dans un fichier
        with open(stats_as_file) as file:  # suppression d'une ligne de la méthode en recourant à la méthode from_string
...
class FootballTeam(SportTeam):  # la classe 'FootballTeam' hérite de la classe 'SportTeam'
    ...
    def stats(self):  # méthode d'instance permettant d'afficher les statitiques de l'équipe
        return f"[FOOTBALL] STATS --> {self.name} a {utilities.pluriel(self.wins, 'victoire')}, " \
               f"{utilities.pluriel(self.losses, 'défaite')} et {utilities.pluriel(self.draws, 'nul')}"
